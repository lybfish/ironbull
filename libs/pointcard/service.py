...
CHANGE_RECHARGE = 1
CHANGE_GIFT = 2
CHANGE_TRANSFER_OUT = 3
CHANGE_TRANSFER_IN = 4
CHANGE_DEDUCT = 5
...
class PointCardService:
    ...
    def recharge_user(
        ...
        self.repo.create(PointCardLog(
            tenant_id=tenant_id,
            user_id=None,
            change_type=CHANGE_DEDUCT if use_self else CHANGE_GIFT,
            ...
            remark="分发给用户" if use_self else "赠送用户",
        ))
        # 流水：用户
        self.repo.create(PointCardLog(
            tenant_id=None,
    ...
    def transfer(
        ...
        if from_self < amount:
            return False, "点卡余额不足", None
        to_self_before = Decimal(str(to_user.point_card_self or 0))
        from_user.point_card_self = from_self - amount
        to_user.point_card_self = to_self_before + amount
        self.member_repo.update_user(from_user)
        self.member_repo.update_user(to_user)
        from_self_after = from_user.point_card_self
        to_self_after = to_user.point_card_self
        self.repo.create(PointCardLog(
            tenant_id=None,
            ...
            after_self=float(from_self_after),
            before_gift=float(from_user.point_card_gift or 0),
            after_gift=float(from_user.point_card_gift or 0),
            remark=f"转出给用户#{to_user_id}",
        ))
        self.repo.create(PointCardLog(
            tenant_id=None,
            user_id=to_user_id,
            ...
            card_type=CARD_SELF,
            amount=float(amount),
            before_self=float(to_self_before),
            after_self=float(to_self_after),
            before_gift=float(to_user.point_card_gift or 0),
            after_gift=float(to_user.point_card_gift or 0),
            remark=f"从用户#{from_user_id}转入",
        ))
        return True, "", {
            "from_user_id": from_user_id,
            "to_user_id": to_user_id,
            "amount": float(amount),
            "from_balance_before": float(from_self),
            "from_balance_after": float(from_self_after),
            "to_balance_before": float(to_self_before),
            "to_balance_after": float(to_self_after),
        }
